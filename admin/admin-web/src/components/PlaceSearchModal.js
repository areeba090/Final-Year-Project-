...
const DEFAULT_CENTER = { lat: 34.1688, lng: 73.2215 };
...
export default function PlaceSearchModal({
  ...
  const handleSearch = () => {
    const query = (searchKeyword || "place").trim();
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-emerald-500/10 to-teal-500/10">
          <h2 id="place-search-title" className="text-xl font-bold text-slate-800">
            {title}
          </h2>
          <button
            type="button"
        ...
        <div className="flex flex-col sm:flex-row flex-1 min-h-0">
          ...
          <div className="w-full sm:w-80 border-t sm:border-t-0 sm:border-l border-slate-200 flex flex-col bg-slate-50/50">
            <div className="p-4 space-y-3">
              <p className="text-xs text-slate-500 font-medium">Search within ~10 km</p>
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="e.g. area name, landmark"
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-4">
              {results.length === 0 && !searching && (
                <p className="text-slate-500 text-sm py-4">Enter a keyword and click Search.</p>
              )}
              {results.length > 0 && (
            ...
            {selectedPlace && (
              <div className="p-4 border-t border-slate-200 bg-white">
                ...
                <button
                  ...
                  className="mt-3 w-full py-2.5 bg-emerald-500 text-white rounded-xl font-semibold text-sm hover:bg-emerald-600"
                >
                  Use this place
                </button>
              </div>
